...
const fallbackProducts = [
  { id: 1, name: 'Modern Bej Salon Halısı', code: 'AH-1024', price: 8500, discountPrice: 7200, isDiscounted: true, isAvailable: true, isFeatured: true, category: 'Salon Halısı', colors: ['Bej', 'Krem'], sizes: ['160x230', '200x300'], style: 'Modern', material: 'Akrilik', description: 'Yumuşak dokulu, geniş salon dekorasyonları için sıcak ve dengeli bir model.' },
  { id: 2, name: 'Vintage Lacivert Yolluk', code: 'AH-1180', price: 3200, discountPrice: null, isDiscounted: false, isAvailable: true, isFeatured: false, category: 'Yolluk', colors: ['Lacivert', 'Gri'], sizes: ['80x150', '120x180'], style: 'Vintage', material: 'Polyester', description: 'Koridor ve giriş alanlarında güçlü desen isteyenler için ince dokulu yolluk.' },
  { id: 3, name: 'Çocuk Odası Renkli Halı', code: 'AH-1302', price: 4100, discountPrice: 3650, isDiscounted: true, isAvailable: false, isFeatured: true, category: 'Çocuk Odası Halısı', colors: ['Gri', 'Yeşil'], sizes: ['120x180', '160x230'], style: 'Minimal', material: 'Pamuk', description: 'Çocuk odaları için kolay uyum sağlayan, yumuşak geçişli ve ferah bir model.' },
]

const fallbackAttributes = {
  categories: ['Salon Halısı', 'Yolluk', 'Çocuk Odası Halısı'],
  colors: ['Bej', 'Krem', 'Gri', 'Lacivert', 'Yeşil'],
  sizes: ['80x150', '120x180', '160x230', '200x300'],
  styles: ['Modern', 'Vintage', 'Minimal'],
  materials: ['Akrilik', 'Polyester', 'Pamuk'],
}
...
function ProductArtwork({ product }) {
  ...
  return (
    <div className="rug-art" aria-hidden="true">
      <span></span>
    </div>
  )
}
  ...
  return (
    <div className="app-shell">
      <header className="site-header">
        ...
        <a className="header-action" href={createWhatsAppLink()} target="_blank" rel="noreferrer">WhatsApp</a>
      </header>

      {route.name === 'home' && (
        <main>
          <section className="hero-section">
            <img src={heroImg} alt="Ay Halı showroom iç mekanında sergilenen halılar" />
            <div className="hero-copy">
              <p>Zonguldak'ın dijital halı showroom'u</p>
              <h1>Kaliteli ve şık halı modellerini mağazaya gelmeden inceleyin.</h1>
              <div className="hero-actions">
                <button type="button" onClick={() => navigate({ name: 'products' })}>Modelleri İncele</button>
                <a href={createWhatsAppLink()} target="_blank" rel="noreferrer">WhatsApp'tan Yaz</a>
              </div>
            </div>
          </section>
        ...
        <main className="catalog-layout">
          ...
          <section className="catalog-results">
            <div className="section-heading inline">
              <div><p>{isLoading ? 'Yükleniyor' : `${visibleProducts.length} model`}</p><h2>Halı modelleri</h2></div>
              <button type="button" onClick={() => setFilters({ ...filters, discounted: true })}>İndirimleri Göster</button>
            </div>
      ...
      {route.name === 'campaigns' && (
        <main className="section page-section">
          <div className="section-heading"><p>Kampanyalar</p><h2>İndirimdeki halılar</h2></div>
      ...
      {route.name === 'detail' && selectedProduct && (
        <main className="detail-layout">
          ...
          <section className="detail-content">
            <button className="back-button" type="button" onClick={() => navigate({ name: 'products' })}>Geri</button>
            ...
            <a className="primary-link" href={createWhatsAppLink(selectedProduct)} target="_blank" rel="noreferrer">Bu ürün için WhatsApp'tan yaz</a>
          </section>
        </main>
      )}
      ...
      {route.name === 'contact' && (
        <main className="contact-page">
          <section>
            ...
            <a className="primary-link" href={createWhatsAppLink()} target="_blank" rel="noreferrer">WhatsApp'tan Yaz</a>
          </section>
          <aside className="contact-panel">
            <strong>Ay Halı Zonguldak</strong>
            <span>Adres: Zonguldak merkez</span>
            <span>Telefon: 0 555 555 55 55</span>
            <span>Instagram: @ayhali</span>
            <span>Çalışma saatleri: 09:00 - 19:00</span>
            <a href="https://www.google.com/maps/search/?api=1&query=Zonguldak%20Ay%20Hali" target="_blank" rel="noreferrer">Yol Tarifi Al</a>
          </aside>
        </main>
      )}

      <footer><span>Ay Halı</span><span>Zonguldak'ta halı modelleri, kampanyalar ve WhatsApp satış desteği.</span></footer>
    </div>
  )
}
      ...
      {products.map((product) => (
        <article className="product-card" key={product.id}>
          ...
          <div className="product-body">
            ...
            <div className="card-actions"><button type="button" onClick={() => onOpen(product)}>Detay</button><a href={createWhatsAppLink(product)} target="_blank" rel="noreferrer">WhatsApp</a></div>
          </div>
        </article>
      ))}
    </div>
